...
// ===== TEST SCHEDULE: 13:15 UTC (14:15 BST) scraper → 13:17/14:17 distributor → 13:20/14:20 delivery =====
cron.schedule('15 13 * * *', async () => {
  console.log('[13:15 UTC] Running scraper...');
  try {
    const http = require('http');
    http.request({ hostname: 'localhost', port: process.env.PORT || 8012, method: 'POST', path: '/api/admin/run-scrapers', headers: { 'Authorization': 'Bearer 9amAdmin2024!', 'Content-Type': 'application/json' } }, function(res) {
      var b = ''; res.on('data', function(c) { b += c; }); res.on('end', function() { console.log('[13:15 UTC] Scraper done'); });
    }).end();
  } catch(e) { console.log('[13:15 UTC] Scraper error:', e.message); }
});
cron.schedule('17 13 * * *', async () => {
  console.log('[13:17 UTC] Distributing...');
  try {
    const http = require('http');
    http.request({ hostname: 'localhost', port: process.env.PORT || 8012, method: 'POST', path: '/api/distribute', headers: { 'Authorization': 'Bearer 9amAdmin2024!', 'Content-Type': 'application/json' } }, function(res) {
      var b = ''; res.on('data', function(c) { b += c; }); res.on('end', function() { console.log('[13:17 UTC] Distributor done'); });
    }).end();
  } catch(e) { console.log('[13:17 UTC] Distributor error:', e.message); }
});
// ===== TEST DELIVERY CRON: Runs directly (not via HTTP) to avoid timing issues =====
// Pipeline: 13:15 UTC scraper → 13:17 UTC distributor → 13:20 UTC delivery
cron.schedule('20 13 * * *', async () => {
  console.log('[13:20 UTC] Running delivery...');
  try {
    _dbData = null;
    var db = getDb();
    var today = new Date().toISOString().split('T')[0];
    ...
    for (var ci = 0; ci < customers.length; ci++) {
      ...
      try {
        ...
        console.log('[13:20 UTC] Delivered ' + custLeads.length + ' to ' + cust.email);
      } catch(e) { console.log('[13:20 UTC] Error for ' + cust.email + ': ' + e.message); }
    }
    console.log('[13:20 UTC] Delivery complete: ' + delivered + ' leads');
  } catch(e) { console.log('[13:20 UTC] Delivery error: ' + e.message); }
});
