...
// ===== TEST SCHEDULE: 10:15 scraper → 10:17 distributor → 10:20 delivery =====
cron.schedule('15 10 * * *', async () => {
  console.log('[10:15 TEST] Scraping fresh leads...');
  try {
    const http = require('http');
    http.request({ hostname: 'localhost', port: process.env.PORT || 8012, method: 'POST', path: '/api/admin/run-scrapers', headers: { 'Authorization': 'Bearer 9amAdmin2024!', 'Content-Type': 'application/json' } }, function(res) {}).end();
  } catch(e) { console.log('[10:15 TEST] Scraper error:', e.message); }
});
cron.schedule('17 10 * * *', async () => {
  console.log('[10:15 TEST] Distributing leads to customers...');
  try {
    const http = require('http');
  ...
  } catch(e) { console.log('[10:15 TEST] Distributor error:', e.message); }
});
cron.schedule('20 10 * * *', async () => {
  console.log('[10:15 TEST] Delivering leads via email...');
  try {
    const http = require('http');
  ...
  } catch(e) { console.log('[10:15 TEST] Delivery error:', e.message); }
});
